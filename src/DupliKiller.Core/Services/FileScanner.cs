...
public class FileScanner : IFileScanner
{
        ...
        ScanMode scanMode = ScanMode.Standard)
    {
        ...
        var zeroByteItems = new ConcurrentBag<FileItem>();
            ...
            .Select(path => Task.Run(() =>
            {
                try
                {
                    ...
                    foreach (var file in dirInfo.EnumerateFiles("*", options))
                    {
                        ...
                        if (file.Length == 0)
                        {
                            zeroByteItems.Add(CreateFileItem(file, file.FullName,
                                FileIssueType.ZeroByte,
        ...
        var corruptedItems = new ConcurrentBag<FileItem>();
        ...
        await Task.Run(() =>
        {
            Parallel.ForEach(candidatePaths, parallelOptions, path =>
            {
                ...
                try
                {
                    ...
                    {
                        Name = info?.Name ?? Path.GetFileName(path),
                        ...
                        Extension = info?.Extension ?? Path.GetExtension(path),
                        QuickHash = primaryHash,
                        FullHash = isDeep ? primaryHash : null,
                        IsSystem = info != null && _systemProtector.IsSystemDirectory(info.FullName),
                        IsHidden = info?.Attributes.HasFlag(FileAttributes.Hidden) ?? false,
                        IsLocked = IsFileLocked(path),
                        Permissions = (info?.Attributes.HasFlag(FileAttributes.ReadOnly) ?? false)
                            ? "Solo lectura"
                            : "Lectura/Escritura"
                    };
                ...
                {
                    Logger.Debug($"Primary hash failed for {path}: {ex.Message}");
                    corruptedItems.Add(CreateFileItem(info, path, FileIssueType.Corrupted,
            ...
            if (collisionTotal > 0)
            {
                ...
                await Task.Run(() =>
                {
                    Parallel.ForEach(collisionFiles, parallelOptions, item =>
                    {
                        ...
                        {
                            Logger.Debug($"FullHash failed for {item.Path}: {ex.Message}");
                            corruptedItems.Add(CreateFileItem(null, item.Path, FileIssueType.Corrupted,
                                $"No se pudo leer el archivo: {ex.Message}"));
                        }
                    });
                }, cancellationToken);
        ...
        // Unique-size files are never hashed, so their readability is verified cheaply here
        // to catch single locked / unreadable files.
        var nonCandidatePaths = sizeGroupDict
            .Where(kvp => kvp.Value.Count == 1)
            .SelectMany(kvp => kvp.Value)
            .ToList();

        if (nonCandidatePaths.Count > 0)
        {
            StatusChanged?.Invoke($"Verificando legibilidad de {nonCandidatePaths.Count} archivos...");

            await Task.Run(() =>
            {
                Parallel.ForEach(nonCandidatePaths, parallelOptions, path =>
                {
                    cancellationToken.ThrowIfCancellationRequested();
                    if (_hashService.CanReadFile(path)) return;
                    ...
                    corruptedItems.Add(CreateFileItem(info, path, FileIssueType.Corrupted,
                        "No se pudo leer el archivo (bloqueado, sin permisos o dañado)"));
                });
            }, cancellationToken);
        }
        ...
        var result = new ScanResult
        {
            ...
            ZeroByteFiles = zeroByteItems.OrderBy(f => f.Path, StringComparer.OrdinalIgnoreCase).ToList(),
            CorruptedFiles = corruptedItems.OrderBy(f => f.Path, StringComparer.OrdinalIgnoreCase).ToList()
        };
        ...
        progress.Report(new ScanStatsUpdate(
            ...
            ZeroByteCount: result.ZeroByteFiles.Count,
            CorruptedCount: result.CorruptedFiles.Count
        ));

        StatusChanged?.Invoke($"Escaneo completado. {finalGroups.Count} grupos duplicados, " +
            $"{result.ZeroByteFiles.Count} vacíos, {result.CorruptedFiles.Count} dañados.");
        PhaseChanged?.Invoke(ScanPhase.Completed, "Escaneo completado.");
        Logger.Info($"Scan complete. {finalGroups.Count} groups, {finalDupCount} dupes, " +
            $"{result.ZeroByteFiles.Count} zero-byte, {result.CorruptedFiles.Count} corrupted.");
        return result;
    }

    private FileItem CreateFileItem(FileInfo? info, string path, FileIssueType issue, string? reason)
    {
        if (info != null)
        {
            return new FileItem
